...
//		g_sprintf(priv->hwIDHex + i * 2, "%02X", priv->hwID[i]);
...
incomingWatch(GSocketService *service, GSocketConnection *connection, GObject *sourceObject, gpointer userData) {
	...
	GSocketFamily socketFamily = g_socket_address_get_family(socketAddr);
	if(socketFamily == G_SOCKET_FAMILY_IPV4) {
		priv->ipBinLen = 4;
	} else if(socketFamily == G_SOCKET_FAMILY_IPV6) {
		priv->ipBinLen = 16;
	} else {
		return FALSE;
	}
	...
	priv->ipBin = g_malloc(priv->ipBinLen * sizeof(guint));
...
static GString* handleMsg(AirplayController *self, gchar *header, gchar *content) {
	...
	if(g_ascii_strncasecmp(header, priv->CMD_OPTIONS, strlen(priv->CMD_OPTIONS)) == 0) {
		...
		if(aesIVStr != NULL) {
			...
			g_free(aesIVStr);
			gint aesIVLength = 0;
			gchar *aesIV =  decodeBase64(aesIVStrTrimed, strlen(aesIVStrTrimed), &aesIVLength);
			...
			gint keyLength = 0;
			gchar *decryptKey = rsaDecrypt(aesKey, aesKeyLength, &keyLength);
			g_free(aesKey);
		...
		priv->renderer = g_object_new(AIRPLAY_TYPE_RENDERER, "aesKey", priv->aesKey, "aesIV", priv->aesIV, NULL);
		...
		g_signal_emit_by_name(self, "record", getRtpInfo(self, header), G_TYPE_NONE);
...
static void writeAppleResponse(AirplayController *self, gchar *header, GString *response) {
	...
	gint pos = 0;
	guchar resp[32];
	memset(resp, 0, sizeof(resp) * sizeof(gchar));
